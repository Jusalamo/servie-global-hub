...
export default function SignUp() {
  ...
  useEffect(() => {
    ...
    if (currentStep === 1) {
      ...
      // Start with 25% for completing step 1
      progress = 25;
      
      // Check name fields (15%)
      if (formData.firstName && formData.lastName) progress += 15;
      
      // Check email (15%)
      if (formData.email && /\S+@\S+\.\S+/.test(formData.email)) progress += 15;
      
      // Check password (15% - partial based on requirements met)
      const passedRequirements = requirements.filter(req => req.regex.test(formData.password)).length;
      progress += (passedRequirements / requirements.length) * 15;
      
      // Check password match (15%)
      if (formData.password && formData.password === formData.confirmPassword) progress += 15;
      
      // Check terms (15%)
      if (formData.agreeToTerms) progress += 15;
    }
    
    setFormProgress(Math.min(100, Math.floor(progress)));
  }, [currentStep, userRole, formData]);
  ...
  const validateRoleSelection = () => {
    if (!userRole) {
      setErrors({ userRole: "Please select a role" });
      return false;
    }
    return true;
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  ...
                  >
                    I agree to the{" "}
                    <a href="/terms" className="text-servie hover:underline">
                      Terms of Service
                    </a>{" "}
                    and{" "}
                    <a href="/privacy" className="text-servie hover:underline">
                      Privacy Policy
                    </a>
                  </Label>
                  {errors.agreeToTerms && (
              ...
              <div className="flex justify-between">
                ...
                <Button 
                  ...
                  disabled={isSubmitting}
                >
                  {isSubmitting ? "Creating Account..." : `Sign up as ${userRole.charAt(0).toUpperCase() + userRole.slice(1)}`}
                </Button>
              </div>
            </form>
          )}
