  ...
  if (layout === "list") {
    return (
      <Card className={`overflow-hidden hover:shadow-lg transition-all duration-300 ${featured ? 'border-servie border-2' : ''}`}>
        <Link to={`/service/${id}`} className="flex flex-col sm:flex-row">
          ...
          <div className="flex flex-col flex-1">
            <CardContent className="p-3 sm:p-4 space-y-2 sm:space-y-3">
              <h3 className="font-semibold text-base sm:text-lg line-clamp-2 whitespace-normal break-words">{title}</h3>
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs sm:text-sm truncate">{providerName}</span>
              </div>
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs sm:text-sm text-muted-foreground truncate">({reviewCount} reviews)</span>
              </div>
            ...
            <CardFooter className="mt-auto px-3 py-2 sm:px-4 sm:py-3 bg-muted/30 flex justify-between items-center gap-2">
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                >
                  <Heart 
                    className={`h-4 w-4 sm:h-5 sm:w-5 ${isFavorite ? 'fill-purple-500 text-purple-500' : 'text-muted-foreground'}`} 
                  />
                </Button>
                <Button size="sm" variant="secondary" className="bg-servie text-white hover:bg-servie-600 text-xs sm:text-sm whitespace-nowrap">View Details</Button>
              </div>
            </CardFooter>
          </div>
        </Link>
  ...
  return (
    <Card className={`overflow-hidden hover:shadow-lg transition-all duration-300 ${featured ? 'border-servie border-2' : 'hover:scale-[1.02]'}`}>
      <Link to={`/service/${id}`} className="block">
        ...
        <CardFooter className="px-3 py-2 sm:px-4 sm:py-3 bg-muted/30 flex justify-between items-center gap-2">
          <span className="font-semibold text-base sm:text-lg whitespace-nowrap">
            {currency}{price}
          </span>
          <Button size="sm" className="bg-servie text-white hover:bg-servie-600 text-xs sm:text-sm whitespace-nowrap">View Details</Button>
        </CardFooter>
      </Link>
    </Card>
  );
